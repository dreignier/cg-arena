...
public class GhostInTheCell extends Engine {
  ...
  PlayerBase playerProcesses[] = new PlayerBase[2];
  ...
  public void initialize() throws Exception {
    ...
    for (int i = 0;i<2;i++) {
      playerProcesses[i] = new PlayerBase(process.get(i), i);

      String[] outputLines = getInitInputForPlayer(i);
      for (String output :outputLines) {
        playerProcesses[i].getOut().println(output);
      }
      playerProcesses[i].getOut().flush();
    }
  }
  ...
  public void play() throws Exception {
    for (int i=0;i<2;i++) {
      ...
      playerProcesses[i].cleanErrorStream();
  ...
  private void addToolTip(int id, String translate) {
    // TODO Auto-generated method stub
    
  }

  private String translate(String string, int... id) {
    return string;
  }
